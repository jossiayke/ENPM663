...
class camera3 : public rclcpp::Node
{
public:
    camera3(std::string node_name) : Node(node_name)
    {
        // initialize the message
        msg_ = sensor_msgs::msg::Image();
               
        // publisher
        publisher_ = this->create_publisher<sensor_msgs::msg::Image>("right", 10);

        // parameters
        this->declare_parameter("frequency", 10.0);
        this->declare_parameter("name", "camera3");
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds((int)(1 / camera_frequency_) * 1000),
                                         std::bind(&camera3::timer_callback, this));
